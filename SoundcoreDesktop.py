...
try:
    ...
    def tryConnect():
        ...
        if data:
            with open(
                os.path.join(os.getcwd(), "lastConnect.json"), "w"
            ) as lastConnect:
                lastConnect.write(json.dumps({"macAddress": inputtxt.get()}))

    connectButton = tkinter.Button(
        window, width=6, height=0, text="connect", command=tryConnect
    )
    ...
    Modeinfo = tkinter.Label(
        window, text="Stay aware of your sourrounding by allowing\n ambient sound in."
    )
    ...
    def ANC():
        ...
        buttons = [("Transport", 1, 30), ("Indoor", 2, 120), ("Outdoor", 3, 210)]
        ...
        def getAction(data=None):
            ...
            if data == 1:
                Modeinfo.configure(
                    text="Targets low-end frequencies like engine and road\n noise for serene journeys and commutes."
                )
                ...
                Modeinfo.configure(
                    text="Eliminate voices and mid-frequency noise from\n coffee shops and other inside spaces."
                )
                ...
                Modeinfo.configure(
                    text="Reduce ambient sound on-the-go for quieter city\n spaces"
                )
        ...
        TransportButton = tkinter.Radiobutton(
            window,
            text=buttons[0][0],
            variable=var,
            command=getAction,
        ...
        IndoorButton = tkinter.Radiobutton(
            window,
            text=buttons[1][0],
            variable=var,
            command=getAction,
        ...
        OutdoorButton = tkinter.Radiobutton(
            window,
            text=buttons[2][0],
            variable=var,
            command=getAction,
    ...
    def trans():
        ...
        forgetANCWegets()
        Modeinfo.configure(
            text="Stay aware of your sourrounding by allowing\n ambient sound in."
        )
        Modeinfo.place(x=45, y=200)
    ...
    TransparcyImg = tkinter.PhotoImage(
        file="Images//Modes//a3029_ambient_icon_trans.png"
    )
    TransparcyImg = TransparcyImg.subsample(2, 2)
    page2btn = tkinter.Button(
        window,
        text="Transparency",
        image=TransparcyImg,
        compound="top",
